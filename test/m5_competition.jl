...
#     fcs = LocalLevelJump(model[:level1], model[:level2], reshape(collect(model[:level_matrix]), (2,2)), model[:level_variance], model[:observation_variance])
#     smc = SMC{SizedVector{3, Float64, Vector{Float64}}, LocalLevelJump}(fcs, 500)

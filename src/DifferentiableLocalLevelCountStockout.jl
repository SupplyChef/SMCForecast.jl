...
but `pi` (a single θ value) and `k` (== the observed count at that
...
continuous level, not just on θ, so `log(lambda)` is recomputed per
...
function differentiable_particle_loglikelihood(::Val{LocalLevelCountStockout}, θ, values, standard_normals::AbstractMatrix;
    ...
    for t in 1:T
        ...
        # density (level2 is a plain θ value, not per-particle), and

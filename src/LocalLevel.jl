...
Local level model
"""
struct LocalLevel <: SMCSystem{MVector{2, Float64}}
    level::Float64
...
function forecast(::Val{LocalLevel}, values, horizon; maxtime=10.0, size=500, forecast_percentiles=0.5)
    ...
    smc = SMC{MVector{2, Float64}, LocalLevel}(fcs, size)
...
function get_loss_function(::Val{LocalLevel}, values; size=100)
    return xs -> begin
        fcs2 = LocalLevel(xs[1], 
                          abs(xs[2]),
                          abs(xs[3]))
        smc = SMC{MVector{2, Float64}, LocalLevel}(fcs2, size)
...
function sample_states(system::LocalLevel, 
                       current_states::Vector{MVector{2, Float64}}, next_observation::Union{Missing, Float64}, 
...
function sample_observation(system::LocalLevel, current_state::MVector{2}; rng=Random.default_rng())
...
function transition_probability(system::LocalLevel, 
                                state::MVector{2, Float64}, 
                                new_observation,
                                new_state::MVector{2, Float64})::Float64
    time = state[1]
...
function observation_probability(system::LocalLevel, state::MVector{2, Float64}, observation::Float64)::Float64

...
function forecast(::Val{LocalLevel}, values, horizon; maxtime=10.0, size=500, forecast_percentiles=0.5)
    ...
    smc = SMC{SizedVector{2, Float64, Vector{Float64}}, LocalLevel}(fcs, size)
...
function sample_initial_state(system::LocalLevel, count)
    states = [SizedVector{2, Float64, Vector{Float64}}(0, r) for r in rand(system.prior_distribution(), count)]
    return states
end
...
function transition_probability(system::LocalLevel, 
                                ...
                                new_observation,
                                new_state::SizedVector{2, Float64, Vector{Float64}})::Float64
    time = state[1]
...
function average_state(system::LocalLevel, states, weights)
    return SizedVector{2, Float64, Vector{Float64}}([states[1][1], sum(states[i][2] * weights[i] for i in eachindex(weights))])
end